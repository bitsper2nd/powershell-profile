...
if ($confirmation -eq 'Y' -or $confirmation -eq 'y') {
    try {
        ...
        if (!(Test-Path -Path $PROFILE -PathType Leaf)) {
            ...
            Write-Host "❇️ If you want to add any persistent components, please do so at [$profilePath\Profile.ps1] as there is an updater in the installed profile which uses the hash to update the profile and will lead to loss of changes"
        } else {
            ...
            Write-Host "❇️ Please back up any persistent components of your old profile to [$HOME\Documents\PowerShell\Profile.ps1] as there is an updater in the installed profile which uses the hash to update the profile and will lead to loss of changes"
        }
...
if ($confirmation -eq 'Y' -or $confirmation -eq 'y') {
    try {
        winget install -e --id Starship.Starship && winget install -e --id ajeetdsouza.zoxide && winget install --id=eza-community.eza && winget install --id=BurntSushi.ripgrep.GNU && winget install --id=sharkdp.bat && winget install --id=sharkdp.fd && winget install --id=chmln.sd && winget install --id=tldr-pages.tlrc
